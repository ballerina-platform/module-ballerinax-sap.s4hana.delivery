...
import sap.s4hana.api_inbounddelivery_srv.mock as _;
...
function testListA_PurchasingInfoRecords() returns error? {
    CollectionOfA_InbDeliveryItemWrapper deliveryitem = check s4HanaClient->listA_InbDeliveryItems();
    test:assertTrue(deliveryitem.d !is (), "The purchasing info record is expected to be non-empty.");
}
...
function testCreateAP() returns error? {
    A_InbDeliveryHeader_1 purchasingInfoRecord = s4HanaClient->createConfirmPutawayAllItems()({
      Header : "80000001"
    });
