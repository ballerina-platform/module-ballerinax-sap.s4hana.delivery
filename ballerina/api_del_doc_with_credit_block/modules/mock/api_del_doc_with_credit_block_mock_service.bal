...
service /sap/opu/odata/sap/API_DEL_DOC_WITH_CREDIT_BLOCK on ep0 {
    ...
    # http:Response (Error)
    resource function get A_CreditBlockedDeliveryDoc(int? \$top, int? \$skip, string? \$filter, "allpages"|"none"? \$inlinecount, A_CreditBlockedDeliveryDocOrderByOptions? \$orderby, A_CreditBlockedDeliveryDocSelectOptions? \$select) returns CollectionOfA_CreditBlockedDeliveryDocWrapper|http:Response {
        return {
            d: {
                results: [
                    {
                        "CreditBlockedDeliveryDoc": "12345"
                    }
                ]
            }
        };
