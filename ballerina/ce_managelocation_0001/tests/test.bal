...
import sap.s4hana.ce_managelocation_0001.mock as _;
...
function testListLocations() returns error? {
    CollectionOfLocation listLocations = check s4HanaClient->listLocations();
    test:assertTrue(listLocations?.value !is (), "The  is expected to be non-empty.");
}

// Since creating a location needs master data. This create response is meant to fail.
@test:Config {
}
